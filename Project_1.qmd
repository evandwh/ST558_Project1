...
```{r}
help_help_codes <- function(geo, geo_name) {
  state_fips <- c(
...
helper_fun(2021, geo = "Region", geo_name = "South")
```

```{r}
help_help(helper_fun(2021, num_vars = c("JWAP", "JWDP")))
```
...
names(jwap_time_lookup) <- sprintf("%03d", c(1:285, 0))
...
names(jwdp_time_lookup) <- sprintf("%03d", c(1:150, 0))
...
interval_midpoint <- function(time_string) {
  ...
  start_just_time <- format(start_time, "%H:%M:%S")
  end_just_time <- format(end_time, "%H:%M:%S")

  midpoint <- (start_just_time + end_just_time) / 2
  midpoint
}
